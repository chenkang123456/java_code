...
<body>
	...
	<div class="container">
		<jsp:include page="comp/nav_comp.jsp">
			<jsp:param value="unresolved" name="from" />
		</jsp:include>
		<!--主体部分-->
		<div class="row">
			<div class="col-md-3">
			...
			<div class="col-md-9">
				<!--未处理问题列表-->
				<div class="problem-title">[首页]>>[未处理问题列表]</div>
				<!--未处理问题-->
				...
				<ul class="pagination">
					<li
						...
						<a
						href="${pageContext.request.contextPath}/problemServlet?flag=unresolved&current=${pageInfo.currentPageNo}&dir=pre">&laquo;</a>
					</li>

					<c:forEach begin="1" end="${pageInfo.pageCount }" var="pi">
						<li
							<c:if test="${pi==pageInfo.currentPageNo }">class="active"</c:if>><a
							href="${pageContext.request.contextPath}/problemServlet?flag=unresolved&pageNo=${pi}">${pi}</a></li>
					</c:forEach>

					<li
						<c:if test="${pageInfo.currentPageNo==pageInfo.pageCount}">class="disabled"</c:if>>
						<a
						href="${pageContext.request.contextPath}/problemServlet?flag=unresolved&current=${pageInfo.currentPageNo}&dir=next">&raquo;</a>
					</li>
				</ul>
			</div>
		</div>
